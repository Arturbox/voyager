...
class VoyagerBaseController extends Controller
{
    ...
    public function restore(Request $request)
    {
        ...
        // Check permission
        $this->authorize('restore', app($dataType->model_name));
    ...
    public function restore_row(Request $request)
    {
        $slug = $this->getSlug($request);
        $request->setMethod('RESTORE');
        $dataType = Voyager::model('DataType')->where('slug', '=', $slug)->first();
        ...
        if (empty($id)) {
            // Bulk delete, get IDs from POST
            $ids = explode(',', $request->restore);
        } else {
            // Single item delete, get ID from URL
            $ids[] = $id;
        }
        foreach ($ids as $id) {
            $data = call_user_func([$dataType->model_name, 'where'], ['id'=>$id]);
            $this->cleanup($dataType, $data);
        }

        $displayName = count($ids) > 1 ? $dataType->display_name_plural : $dataType->display_name_singular;

        $res = $data->restore($ids);
